...
const fs = require('fs');
const Turkium = require('../../../../nodejs/Turkium-dev');
const {
    Wallet, setDefaultStorageFolder,
    AccountKind, Mnemonic, Resolver,
    TurkiumToSompi,
    sompiToTurkiumString,
    Address
} = Turkium;
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToTurkiumString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        ...
                        list.push({
                            ...
                            Mature: sompiToTurkiumString(b.mature),
                            Pending: sompiToTurkiumString(b.pending),
                            Outgoing: sompiToTurkiumString(b.outgoing),
        ...
        // Send Turkium to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeSompi: TurkiumToSompi("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: TurkiumToSompi("1.567")
            }]
        });
        ...
        // Transfer Turkium between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: TurkiumToSompi("2.456"),
        });

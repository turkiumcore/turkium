...
- [**Rust** documentation](https://docs.rs/Turkium-wasm/latest/Turkium_wasm/index.html)
- [**TypeScript** documentation](https://Turkium.aspectron.org/docs/)
...
- `web/Turkium` - **full** Rusty Turkium WASM32 SDK bindings for use in web browsers.
- `web/Turkium-rpc` - only the RPC bindings for use in web browsers (reduced WASM binary size).
- `nodejs/Turkium` - **full** Rusty Turkium WASM32 SDK bindings for use with NodeJS.
- `docs` - Rusty Turkium WASM32 SDK documentation.
...
- `examples/javascript/wallet` - Interfacing with the Rusty Turkium Wallet framework.
...
Included documentation in the release can be accessed by loading the `docs/Turkium/index.html` 
...
before loading the WASM32 library. The compatible WebSocket library is [WebSocket](https://www.npmjs.com/package/websocket) and is included in the `Turkium` NPM package. `Turkium` package is a wrapper around `Turkium-wasm` that imports and installs this WebSocket shim in the `globalThis` object and then re-exports `Turkium-wasm` exports.

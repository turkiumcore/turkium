...
This Stratum Bridge is currently in BETA. Support is available in the Turkium Discord’s [#mining-and-hardware](https://discord.com/channels/599153230659846165/910178666099646584) channel.

For bug reports or feature request, please open an issue at [`turkiumnet/rusty-turkium` issues](https://github.com/turkiumnet/rusty-turkium/issues) and prefix your issue title with `[Bridge]`.
...
- **External** node (you run `Turkiumd` yourself)
- **In-process** node (the bridge starts `Turkiumd` in the same process)
...
This will show all available bridge options and guidance for Turkiumd arguments.
...
- Default `Turkiumd_address`: `localhost:16110` (code default) or `127.0.0.1:16110` (as in `config.yaml`)
...
Run in-process with explicit config and Turkiumd args:

```bash
cargo run -p Turkium-stratum-bridge --release --bin stratum-bridge -- --config bridge/config.yaml --node-mode inprocess -- --utxoindex --rpclisten=127.0.0.1:16110
```

**Important:** Use `--` separator before Turkiumd arguments. Arguments starting with hyphens must come after the `--` separator.

**Examples:**
```bash
# ✓ Correct - bridge args first, then --, then Turkiumd args
...
**Note:** In-process mode uses a separate app directory by default to avoid RocksDB lock conflicts with an existing `Turkiumd`.
...
```bash
cargo run --release --bin Turkiumd -- --utxoindex --rpclisten=127.0.0.1:16110
```

Terminal B (bridge):

```bash
cargo run -p Turkium-stratum-bridge --release --bin stratum-bridge -- --config bridge/config.yaml --node-mode external
```
...
```bash
cargo run -p Turkium-stratum-bridge --release --features rkstratum_cpu_miner --bin stratum-bridge -- --config config.yaml --web-dashboard-port :3031 --node-mode external --Turkiumd-address 127.0.0.1:16210 --instance "port=:16120,diff=1" --internal-cpu-miner --internal-cpu-miner-address "Turkiumtest:address" --internal-cpu-miner-threads 1
```
...
- **Username / wallet:** `turkium:YOUR_WALLET_ADDRESS.WORKERNAME`
...
```powershell
$env:RUST_LOG="info,Turkium_stratum_bridge=debug"
```
...
```bash
cargo build -p Turkium-stratum-bridge --release --features rkstratum_cpu_miner
```

Run (external node mode + internal CPU miner enabled):

```bash
cargo run -p Turkium-stratum-bridge --release --features rkstratum_cpu_miner --bin stratum-bridge -- --config bridge/config.yaml --node-mode external --internal-cpu-miner --internal-cpu-miner-address turkium:YOUR_WALLET_ADDRESS --internal-cpu-miner-threads 1
```
...
```bash
cargo test -p Turkium-stratum-bridge --features rkstratum_cpu_miner --bin stratum-bridge
```

Run tests without the CPU miner feature:

```bash
cargo test -p Turkium-stratum-bridge --bin stratum-bridge
```

...
use turkium_stratum_bridge::log_colors::LogColors;
use turkium_stratum_bridge::{StratumServerBridgeConfig as StratumBridgeConfig, TurkiumApi, listen_and_serve_with_shutdown, prom};
use clap::Parser;
...
fn log_bridge_configuration(config: &BridgeConfig) {
    ...
    tracing::info!("\tturkiumd:          {} (shared)", config.global.turkiumd_address);
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    prom::set_web_status_config(config.global.turkiumd_address.clone(), config.instances.len());
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        if node_mode == NodeMode::Inprocess {
            EnvFilter::new("warn,turkium_stratum_bridge=info,Turkium=info,turkiumd=info,turkiumd_lib=info,log=info")
        } else {
            EnvFilter::new("warn,turkium_stratum_bridge=info")
        }
    });
    ...
    // are not filtered out by a tracing subscriber installed by turkiumd.
    let mut inprocess_node: Option<InProcessNode> = None;
    if node_mode == NodeMode::Inprocess {
        let mut node_args: Vec<String> = cli.turkiumd_args;

        if cli.testnet && !node_args.iter().any(|arg| arg == "--testnet") {
            node_args.push("--testnet".to_string());
        }

        // Add appdir if not provided in turkiumd_args
        if !node_args.iter().any(|arg| arg.starts_with("--appdir")) {
            let default_appdir = app_dirs::default_inprocess_turkiumd_appdir();
            ...
            assert!(cli.appdir.is_none(), "appdir should not be specified both in bridge args and turkiumd args");
        }

        let mut argv: Vec<OsString> = Vec::with_capacity(node_args.len() + 1);
        argv.push(OsString::from("turkiumd"));
        argv.extend(node_args.iter().map(OsString::from));
        let args = turkiumd_args::Args::parse(argv).map_err(|e| anyhow::anyhow!("{}", e))?;
    ...
    // Create shared Turkium API client (all instances use the same node)
    let turkium_api =
        TurkiumApi::new(config.global.turkiumd_address.clone(), config.global.coinbase_tag_suffix.clone(), shutdown_rx.clone())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create Turkium API client: {}", e))?;
    ...
    #[cfg(feature = "rkstratum_cpu_miner")]
    {
        if cli.internal_cpu_miner {
            let mining_address = cli
                ...
                .clone()
                .ok_or_else(|| anyhow::anyhow!("--internal-cpu-miner requires --internal-cpu-miner-address <turkium:...>"))?;
            ...
            let cfg = turkium_stratum_bridge::InternalCpuMinerConfig {
                enabled: true,
                mining_address,
                threads,
                throttle,
            ...
            turkium_stratum_bridge::prom::set_internal_cpu_mining_address(cfg.mining_address.clone());

            let metrics = turkium_stratum_bridge::spawn_internal_cpu_miner(Arc::clone(&turkium_api), cfg, shutdown_rx.clone())?;
            turkium_stratum_bridge::set_rkstratum_cpu_miner_metrics(metrics);
            ...
            // This is best-effort and does not affect mining.
            {
                ...
                let internal_metrics = turkium_stratum_bridge::RKSTRATUM_CPU_MINER_METRICS.lock().as_ref().cloned();
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        turkium_stratum_bridge::prom::record_internal_cpu_miner_snapshot(
                            hashes_tried,
                            blocks_submitted,
                            blocks_accepted,
                            hashrate_ghs,
    ...
    for (idx, instance_config) in config.instances.iter().enumerate() {
        ...
        let turkium_api_clone = Arc::clone(&turkium_api);
        ...
        let handle = tokio::spawn(async move {
            ...
            let bridge_config = StratumBridgeConfig {
                ...
                turkiumd_address: global.turkiumd_address.clone(),
            ...
            listen_and_serve_with_shutdown(
                bridge_config,
                Arc::clone(&turkium_api_clone),
                if is_first_instance { Some(turkium_api_clone) } else { None },

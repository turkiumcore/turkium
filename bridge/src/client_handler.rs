...
impl ClientHandler {
    ...
    pub async fn send_immediate_job_to_client<T: TurkiumApiTrait + Send + Sync + ?Sized + 'static>(
        &self,
        ...
        tokio::spawn(async move {
            ...
            // Get block template
            let template_result = turkium_api_clone.get_block_template(&wallet_addr, &remote_app, &canxium_addr).await;
    ...
    pub async fn new_block_available<T: TurkiumApiTrait + Send + Sync + 'static>(&self, turkium_api: Arc<T>) {
            ...
            tokio::spawn(async move {
                ...
                let block = match template_result {
                    ...
                    Err(e) => {
                        if e.to_string().contains("Could not decode address") {
                            record_worker_error(&instance_id, &wallet_addr, crate::errors::ErrorShortCode::InvalidAddressFmt.as_str());
                            error!("failed fetching new block template from Turkium, malformed address: {}", e);
                            client_clone.disconnect();
                        } else {
                            record_worker_error(&instance_id, &wallet_addr, crate::errors::ErrorShortCode::FailedBlockFetch.as_str());
                            error!("failed fetching new block template from turkium: {}", e);
                        }
                        return;
                    }
                };
        ...
        // Check balances periodically
        {
            ...
            if last_check.elapsed() > BALANCE_DELAY && !addresses.is_empty() {
                *last_check = Instant::now();
                ...
                // Fetch balances via turkium_api
                let addresses_clone = addresses.clone();
                let turkium_api_clone = Arc::clone(&turkium_api);
                let instance_id = self.instance_id.clone();
                tokio::spawn(async move {
                    match turkium_api_clone.get_balances_by_addresses(&addresses_clone).await {
                        Ok(balances) => {
                            // Record balances
                            crate::prom::record_balances(&instance_id, &balances);
                        }
                        Err(e) => {
                            warn!("failed to get balances from Turkium, prom stats will be out of date: {}", e);
                        }
                    }
                });
            }

...
/// Start block template listener with concrete TurkiumApi
...
pub async fn start_block_template_listener_with_api(
    ...
    let turkium_api_cb = Arc::clone(&turkium_api);

    let block_cb = move || {
        let client_handler = Arc::clone(&client_handler_cb);
        let turkium_api = Arc::clone(&turkium_api_cb);
        tokio::spawn(async move {
            client_handler.new_block_available(turkium_api).await;
        });
    };
...
pub async fn listen_and_serve<T: TurkiumApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    turkium_api: Arc<T>,
    // Optional: if concrete TurkiumApi is provided, use notification-based listener
    concrete_turkium_api: Option<Arc<TurkiumApi>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, turkium_api, concrete_turkium_api, None).await
}

pub async fn listen_and_serve_with_shutdown<T: TurkiumApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    turkium_api: Arc<T>,
    concrete_turkium_api: Option<Arc<TurkiumApi>>,
    shutdown_rx: watch::Receiver<bool>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    listen_and_serve_impl(config, turkium_api, concrete_turkium_api, Some(shutdown_rx)).await
}

async fn listen_and_serve_impl<T: TurkiumApiTrait + Send + Sync + 'static>(
    config: BridgeConfig,
    turkium_api: Arc<T>,
    concrete_turkium_api: Option<Arc<TurkiumApi>>,
    ...
    let authorize_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            let event_clone = event.clone();
            Box::pin(async move {
                handle_authorize(ctx_clone, event_clone, Some(client_handler), Some(turkium_api))
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    let submit_handler = {
        ...
        Arc::new(move |ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                share_handler
                    .handle_submit(ctx_clone, event, turkium_api)
                    .await
                    .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
            })
                as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
    ...
    // If concrete TurkiumApi is provided, use notification-based listener
    // Otherwise, use polling only (fallback for trait objects)
    if let Some(concrete_api) = concrete_turkium_api {
        // We have concrete TurkiumApi - use notification-based listener
        let client_handler_cb = Arc::clone(&client_handler);
        let turkium_api_cb = Arc::clone(&turkium_api);

        let block_cb: Box<dyn Fn() + Send + 'static> = Box::new(move || {
            let client_handler = Arc::clone(&client_handler_cb);
            let turkium_api = Arc::clone(&turkium_api_cb);
            tokio::spawn(async move {
                client_handler.new_block_available(turkium_api).await;
        ...
        // Call the method directly on Arc<TurkiumApi> (it's an instance method taking Arc<Self>)
        ...
        // No concrete TurkiumApi provided - use polling only
        warn!("Using polling-based block template listener (concrete TurkiumApi not provided, notifications not available)");

        let client_handler_poll = Arc::clone(&client_handler);
        let turkium_api_poll = Arc::clone(&turkium_api);
            ...
            loop {
                if let Some(ref mut rx) = shutdown_rx_poll {
                    tokio::select! {
                        ...
                        _ = interval.tick() => {
                            client_handler_poll.new_block_available(Arc::clone(&turkium_api_poll)).await;
                        }
                    }
                } else {
                    interval.tick().await;

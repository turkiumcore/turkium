    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        // Special-case forwarded node logs (from `tracing_log::LogTracer`) to match turkiumd style:
        ...
        if self.apply_colors {
            ...
            if message.contains("[ASIC->BRIDGE]") {
                ...
                // Configuration lines - color the label part (e.g., "\tturkiumd:          value")
...
pub(crate) fn init_tracing(
    ...
    // The embedded turkiumd runtime initializes the global `log` logger via
    // `turkium_core::log::init_logger(...).unwrap()`. If LogTracer grabs the
    // global logger first, turkiumd panics with SetLoggerError.

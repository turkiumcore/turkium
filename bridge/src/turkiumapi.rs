...
use crate::share_handler::TurkiumApiTrait;
use turkium_addresses::Address;
use turkium_consensus_core::block::Block;
use turkium_grpc_client::GrpcClient;
use turkium_notify::{listener::ListenerId, scope::NewBlockTemplateScope};
...
impl TurkiumApi {
    /// Create a new Turkium API client
    pub async fn new(
        address: String,
        coinbase_tag_suffix: Option<String>,
        mut shutdown_rx: watch::Receiver<bool>,
    ) -> Result<Arc<Self>> {
        info!("Connecting to turkium node at {}", address);
        ...
        debug!("{} {}", LogColors::api("[API]"), LogColors::label("Establishing RPC connection to turkium node:"));
        ...
        let client = loop {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    warn!(
                        "failed to connect to turkium node at {} (attempt {}): {}, retrying in {:.2}s",
                        grpc_address,
                        attempt,
                        e,
                        backoff.as_secs_f64()
    ...
    async fn start_stats_thread(self: Arc<Self>) {
        ...
        use turkium_rpc_core::{EstimateNetworkHashesPerSecondRequest, GetBlockDagInfoRequest};
        ...
        loop {
            ...
            let dag_response = match self.client.get_block_dag_info_call(None, GetBlockDagInfoRequest {}).await {
                Ok(r) => r,
                Err(e) => {
                    warn!("failed to get network hashrate from Turkium, prom stats will be out of date: {}", e);
                    continue;
                }
            };
    ...
    pub async fn submit_block(&self, block: Block) -> Result<SubmitBlockResponse> {
        // Use turkium_consensus_core::hashing::header::hash() for block hash calculation
        // In Turkium, the block hash is the header hash (transactions are represented by hash_merkle_root in header)
        use turkium_consensus_core::hashing::header;
        ...
        debug!(
            "{} {}",
            LogColors::api("[API]"),
            LogColors::api(&format!("===== ATTEMPTING BLOCK SUBMISSION TO TURKPA NODE ===== Hash: {}", block_hash))
        );
            ...
            Ok(response) => {
                ...
                if !response.report.is_success() {
                    ...
                    warn!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::validation(&format!("===== BLOCK REJECTED BY TURKPA NODE ===== Hash: {}", block_hash))
                    );
                    warn!(
                        "{} {} {}",
                        LogColors::api("[API]"),
                ...
                info!(
                    "{} {}",
                    LogColors::api("[API]"),
                    LogColors::block(&format!("===== BLOCK ACCEPTED BY TURKPA NODE ===== Hash: {}", block_hash))
                );
            ...
            Err(e) => {
                ...
                if error_str.contains("ErrDuplicateBlock") || error_str.contains("duplicate") {
                    warn!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::validation(&format!("===== BLOCK REJECTED BY TURKPA NODE: STALE ===== Hash: {}", block_hash))
                    );
                    warn!(
                        "{} {} {}",
                        LogColors::api("[API]"),
                    ...
                    error!(
                        "{} {}",
                        LogColors::api("[API]"),
                        LogColors::error(&format!("===== BLOCK REJECTED BY TURKPA NODE: INVALID ===== Hash: {}", block_hash))
                    );
    ...
    pub async fn wait_for_sync_with_shutdown(&self, mut shutdown_rx: watch::Receiver<bool>) -> Result<()> {
        debug!("checking turkiumd sync state");
        ...
        loop {
            ...
            match sync_res {
                Ok(is_synced) => {
                    if is_synced {
                        debug!("turkiumd synced, starting server");
                        break;
                    }
                }
                Err(e) => {
            ...
            warn!("Turkium is not synced, waiting for sync before starting bridge");
                ...
                Err(e) => {
                    ...
                    if error_str.contains("Odd number of digits") {
                        return Err(anyhow::anyhow!(
                            "Failed to convert RPC block to Block after {} attempts: {} - This usually indicates a malformed hash field in the block template from the turkium node. The block may have a hash with an odd-length hex string.",
                            max_retries,
                            error_str
                        ));
                    } else {
    ...
    pub async fn get_balances_by_addresses(&self, addresses: &[String]) -> Result<Vec<(String, u64)>> {
        ...
        let utxos = self
            .client
            .get_utxos_by_addresses_call(None, turkium_rpc_core::GetUtxosByAddressesRequest::new(addresses))
            .await
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = api_clone.wait_for_sync().await {
                    error!("error checking turkiumd sync state, attempting reconnect: {}", e);
...
impl TurkiumApiTrait for TurkiumApi {
    async fn get_block_template(
        ...
        TurkiumApi::get_block_template(self, wallet_addr, "", "").await.map_err(|e| {
    ...
    async fn submit_block(
        &self,
        block: Block,
    ) -> Result<turkium_rpc_core::SubmitBlockResponse, Box<dyn std::error::Error + Send + Sync>> {
        TurkiumApi::submit_block(self, block)
            .await
    ...
    async fn get_balances_by_addresses(
        ...
        TurkiumApi::get_balances_by_addresses(self, addresses)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }

    async fn get_current_block_color(&self, block_hash: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        TurkiumApi::get_current_block_color(self, block_hash)
            .await
            .map_err(|e| Box::new(std::io::Error::other(e.to_string())) as Box<dyn std::error::Error + Send + Sync>)
    }
}

...
use turkium_stratum_bridge::{BridgeConfig, InstanceConfig, net_utils::normalize_port};
...
pub struct Cli {
    ...
    #[arg(last = true, help = "turkiumd arguments (use '--' separator if turkiumd args start with hyphens)")]
    pub turkiumd_args: Vec<String>,

    #[arg(long)]
    pub turkiumd_address: Option<String>,
...
pub fn apply_cli_overrides(config: &mut BridgeConfig, cli: &Cli) -> Result<(), anyhow::Error> {
    if let Some(addr) = cli.turkiumd_address.as_deref() {

...
pub fn init_metrics() {
    ...
    BLOCK_ACCEPTED_COUNTER.get_or_init(|| {
        register_counter_vec!(
            ...
            "Number of blocks accepted by the connected turkium node (may later be red)",
            WORKER_LABELS
        )
        .unwrap()
    });
    ...
    #[cfg(feature = "rkstratum_cpu_miner")]
    {
        ...
        INTERNAL_CPU_BLOCKS_ACCEPTED_TOTAL.get_or_init(|| {
            register_counter!(
                ...
                "Total blocks accepted by the connected turkium node from the internal CPU miner since process start"
            )
            .unwrap()
        });
...
async fn handle_http_request(
    ...
    if request.starts_with("GET /api/status") {
        let turkiumd_version = crate::turkiumapi::NODE_STATUS.lock().server_version.clone().unwrap_or_else(|| "-".to_string());
        ...
        let status = WebStatusResponse {
            turkiumd_address: status_cfg.turkiumd_address,
            turkiumd_version,
...
struct WebStatusResponse {
    turkiumd_address: String,
    turkiumd_version: String,
    instances: usize,
    web_bind: String,
}
...
pub fn set_web_status_config(turkiumd_address: String, instances: usize) {
    let lock = WEB_STATUS_CONFIG
        .get_or_init(|| parking_lot::RwLock::new(WebStatusConfig { turkiumd_address: "-".to_string(), instances: 1 }));
    *lock.write() = WebStatusConfig { turkiumd_address, instances: instances.max(1) };
}
...
pub fn record_balances(instance_id: &str, balances: &[(String, u64)]) {
    if let Some(gauge) = BALANCE_GAUGE.get() {
        for (address, balance) in balances {
            // Convert from sompi to TURK (divide by 100000000)
            let balance_turk = *balance as f64 / 100_000_000.0;
            gauge.with_label_values(&[instance_id, address]).set(balance_turk);
        }
    }
}
...
async fn get_config_json() -> String {
        ...
        && let Ok(config) = BridgeConfig::from_yaml(&content)
    {
        ...
        let json_value = serde_json::json!({
            // Global fields
            "turkiumd_address": config.global.turkiumd_address,
...
async fn update_config_from_json(json_body: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(addr) = updates.get("turkiumd_address").and_then(|v| v.as_str()) {
        config.global.turkiumd_address = addr.to_string();
    }
...
"#,
        ...
        assert!(status_resp.contains("\"turkiumd_address\""));
        ...
        assert!(config_resp.contains("\"turkiumd_address\""));
        ...
        let json_body = r#"{"turkiumd_address":"127.0.0.2:16110","stratum_port":":5556","min_share_diff":4096}"#;

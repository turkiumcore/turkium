...
// turkium_pow used inline for PoW validation
...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        event: JsonRpcEvent,
        turkium_api: Arc<dyn TurkiumApiTrait + Send + Sync>,
        ...
        if let Some(Value::String(submitted_identity)) = event.params.first() {
            ...
            let submitted_clean = submitted_address.trim_start_matches("turkium:").trim_start_matches("Turkiumtest:");
            let authorized_clean = wallet_addr.trim_start_matches("turkium:").trim_start_matches("Turkiumtest:");
        ...
        loop {
            ...
            // Use turkium_pow::State for PoW validation against the header's compact bits target.
            ...
            // IMPORTANT: Use turkium_pow's own compact-target handling as the source of truth.
            ...
            debug!("[SUBMIT]   - check_passed(turkium_pow): {}", check_passed);
            ...
            if meets_network_target {
                ...
                // The header comes directly from the turkium node via get_block_template_call()
                // We preserve: version, bits, timestamp, all hash fields, parents, scores, etc.
                header_clone.nonce = nonce_val;

                // Verify timestamp is still valid (not too old)
                // Turkium typically accepts blocks with timestamps within a reasonable window
                ...
                debug!(
                    ...
                    "Using REAL header from turkium node block template"
                );
                ...
                debug!("{} {}", LogColors::block("[BLOCK]"), "Calling turkium_api.submit_block()...");

                // Submit block to node
                let block_submit_result = turkium_api.submit_block(block.clone()).await;
                ...
                match block_submit_result {
                    Ok(response) => {
                        ...
                        let turkium_api = Arc::clone(&turkium_api);
                        let block_hash_for_confirm = block_hash.clone();

                        tokio::spawn(async move {
                            for _ in 0..BLOCK_CONFIRM_MAX_ATTEMPTS {
                                match turkium_api.get_current_block_color(&block_hash_for_confirm).await {
                                    Ok(true) => {
    ...
    fn start_print_stats_thread_impl(&self, target_spm: u32, shutdown_rx: Option<watch::Receiver<bool>>) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let peers = node_status.peers.map(|p: usize| p.to_string()).unwrap_or_else(|| "-".to_string());
                let vdaa = node_status.virtual_daa_score.map(|v: u64| v.to_string()).unwrap_or_else(|| "-".to_string());
                let blocks = node_status.block_count.map(|v: u64| v.to_string()).unwrap_or_else(|| "-".to_string());
                let headers = node_status.header_count.map(|v: u64| v.to_string()).unwrap_or_else(|| "-".to_string());
                let diff = node_status.difficulty.map(|d: f64| format!("{:.2e}", d)).unwrap_or_else(|| "-".to_string());
                let tip = node_status.tip_hash.as_deref().unwrap_or("-");
                let mempool = node_status.mempool_size.map(|v: u64| v.to_string()).unwrap_or_else(|| "-".to_string());
                ...
                let net_short = {
                    ...
                    if let Some(pos) = net.find("network_type:") {
                        ...
                        let s = s.trim_start();
                        network_type = s.split(&[',', '}'][..]).next().map(|v: &str| v.trim());
                    }
                    if let Some(pos) = net.find("suffix:") {
                        let s = &net[pos + "suffix:".len()..];
                        let s = s.trim_start();
                        let raw = s.split(&[',', '}'][..]).next().map(|v: &str| v.trim());
                        if raw != Some("None") {
                            suffix = raw;
                        }
                    }
...
// Trait for Turkium API operations
#[async_trait::async_trait]
pub trait TurkiumApiTrait: Send + Sync {

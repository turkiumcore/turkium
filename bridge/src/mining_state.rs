...
pub struct MiningState {
    ...
    stratum_diff: Arc<Mutex<Option<TurkiumDiff>>>,
    max_jobs: u16,
    last_header: Arc<Mutex<Option<turkium_consensus_core::header::Header>>>, // Track previous header for change logging
}
...
impl MiningState {
    ...
    pub fn stratum_diff(&self) -> Option<TurkiumDiff> {
        self.stratum_diff.lock().clone()
    }

    /// Set stratum difficulty
    pub fn set_stratum_diff(&self, diff: TurkiumDiff) {
    ...
    /// Get last header
    pub fn get_last_header(&self) -> Option<turkium_consensus_core::header::Header> {
        self.last_header.lock().clone()
    }

    /// Set last header
    pub fn set_last_header(&self, header: turkium_consensus_core::header::Header) {

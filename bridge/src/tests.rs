...
#[cfg(test)]
use turkium_stratum_bridge::BridgeConfig;
...
fn test_config_single_instance_mode() {
    ...
    let yaml = r#"
turkiumd_address: "127.0.0.1:5200"
stratum_port: ":5555"
min_share_diff: 8192
print_stats: true
"#;
    ...
    assert_eq!(config.global.turkiumd_address, "127.0.0.1:5200", "turkiumd address should be stored in global config");
}
...
fn test_config_single_instance_log_to_file_uses_global() {
    ...
    let yaml = r#"
turkiumd_address: "127.0.0.1:16110"
stratum_port: ":5555"
min_share_diff: 8192
log_to_file: false
"#;
...
fn test_normalize_port_with_colon() {
    ...
    use turkium_stratum_bridge::net_utils::normalize_port;
...
fn test_bind_addr_from_port_with_colon() {
    use turkium_stratum_bridge::net_utils::bind_addr_from_port;
...
fn test_stratum_method_from_str() {
    use turkium_stratum_bridge::jsonrpc_event::StratumMethod;
...
fn test_jsonrpc_event_new() {
    use turkium_stratum_bridge::jsonrpc_event::JsonRpcEvent;
    use serde_json::json;
...
fn test_jsonrpc_event_method_enum() {
    use turkium_stratum_bridge::jsonrpc_event::{JsonRpcEvent, StratumMethod};
...
fn test_jsonrpc_response_new() {
    use turkium_stratum_bridge::jsonrpc_event::{JsonRpcEvent, JsonRpcResponse};
    use serde_json::json;
...
fn test_jsonrpc_response_success() {
    use turkium_stratum_bridge::jsonrpc_event::JsonRpcResponse;
...
fn test_unmarshal_event_basic() {
    use turkium_stratum_bridge::jsonrpc_event::unmarshal_event;
...
fn test_unmarshal_response_success() {
    use turkium_stratum_bridge::jsonrpc_event::unmarshal_response;
...
fn test_error_short_code_display() {
    use turkium_stratum_bridge::errors::ErrorShortCode;
...
fn test_mining_state_initial_values() {
    ...
    // - No stored job IDs
    use turkium_stratum_bridge::mining_state::MiningState;
...
fn test_mining_state_job_management() {
    ...
    use turkium_stratum_bridge::mining_state::{Job, MiningState};
...
// These tests run with: cargo test -p Turkium-stratum-bridge --bin stratum-bridge
// Or with CPU miner: cargo test -p Turkium-stratum-bridge --features rkstratum_cpu_miner --bin stratum-bridge

#[cfg(test)]
mod integration {
    use turkium_alloc::init_allocator_with_default_settings;
    use turkium_stratum_bridge::{StratumServerBridgeConfig as StratumBridgeConfig, TurkiumApi, listen_and_serve_with_shutdown};
    use turkiumd_lib::args as turkiumd_args;
    ...
    async fn test_bridge_startup_with_inprocess_node() {
        ...
        let temp_dir = std::env::temp_dir().join(format!("turkiumd_test_{}", uuid::Uuid::new_v4()));
        ...
        let bridge_handle =
            tokio::spawn(
                async move { listen_and_serve_with_shutdown::<TurkiumApi>(bridge_config, turkium_api, None, shutdown_rx).await },
            );
    ...
    async fn test_bridge_startup_with_cpu_miner_feature() {
        ...
        use turkium_stratum_bridge::InternalCpuMinerConfig;
        let miner_config = InternalCpuMinerConfig {
            enabled: false, // Don't actually mine in test
            mining_address: "Turkiumtest:test".to_string(),
            threads: 1,
            throttle: None,
...
mod comprehensive_tests {
    ...
    use turkium_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionOutput};
    use turkium_hashes::Hash;
    use turkium_stratum_bridge::{
    ...
    /// This helps us test share validation without needing a real turkium node
    ...
    // - turkiumDiff: Stores difficulty, target, and hash value
    // ========================================================================
    ...
    fn test_difficulty_to_target_calculation() {
        ...
        use turkium_stratum_bridge::hasher::diff_to_target;
    ...
    #[test]
    fn test_turkium_diff_structure() {
        // Test: TurkiumDiff structure and methods
        // This demonstrates how difficulty is stored and managed

        let mut diff = TurkiumDiff::new();
    ...
    fn test_difficulty_for_different_miners() {
        ...
        let mut diff1 = TurkiumDiff::new();
        let mut diff2 = TurkiumDiff::new();
        let mut diff3 = TurkiumDiff::new();
    ...
    fn test_mining_state_difficulty_storage() {
        ...
        let mut stratum_diff = TurkiumDiff::new();
    ...
    fn test_share_handler_worker_stats() {
        ...
        *ctx.wallet_addr.lock() = "Turkiumtest:test".to_string();
    ...
    // Run with: cargo test -p Turkium-stratum-bridge --features rkstratum_cpu_miner
    // ========================================================================
    ...
    fn test_internal_cpu_miner_config_creation() {
        ...
        let config = InternalCpuMinerConfig {
            enabled: true,
            mining_address: "Turkiumtest:test123456789012345678901234567890123456789012345678901234567890".to_string(),
            threads: 4,
        ...
        assert_eq!(config.mining_address, "Turkiumtest:test123456789012345678901234567890123456789012345678901234567890");
    ...
    fn test_internal_miner_metrics_initialization() {
        ...
        use turkium_stratum_bridge::rkstratum_cpu_miner::InternalMinerMetrics;
    ...
    async fn test_internal_cpu_miner_spawn_disabled() {
        ...
        // Create a mock TurkiumApi (we won't actually use it when disabled)
        ...
        // without needing a real TurkiumApi
    ...
    async fn test_internal_cpu_miner_integration_with_mock_api() {
        ...
        // Note: Full integration requires a real turkium node (tested in integration tests)
        ...
        // let metrics = spawn_internal_cpu_miner(turkium_api, config, shutdown_rx).await?;
    ...
    fn test_internal_cpu_miner_pow_checking_concept() {
        ...
        use turkium_consensus_core::header::Header;
        use turkium_hashes::Hash;
        use turkium_pow::State as PowState;
    ...
    async fn test_wallet_address_cleaning_with_prefix() {
        // Test: Wallet addresses with turkium:/Turkiumtest:/turkiumdev: prefixes
        ...
        let event1 = JsonRpcEvent::new(
            Some("1".to_string()),
            "mining.authorize",
            vec![json!("turkium:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        ...
        assert!(addr1.starts_with("turkium:"), "Address should have turkium: prefix");
        ...
        // Test Turkiumtest: prefix
        let ctx2 = create_test_context().await;
        let event2 = JsonRpcEvent::new(
            Some("2".to_string()),
            "mining.authorize",
            vec![json!("Turkiumtest:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event2.params.len(), 1);
        let addr2 = event2.params[0].as_str().unwrap();
        assert!(addr2.starts_with("Turkiumtest:"), "Address should have Turkiumtest: prefix");
        let _result2: Result<(), _> = handle_authorize(ctx2.clone(), event2, None, None).await;

        // Test turkiumdev: prefix
        let ctx3 = create_test_context().await;
        let event3 = JsonRpcEvent::new(
            Some("3".to_string()),
            "mining.authorize",
            vec![json!("turkiumdev:qr8example123456789012345678901234567890123456789012345678901234567890")],
        );
        assert_eq!(event3.params.len(), 1);
        let addr3 = event3.params[0].as_str().unwrap();
        assert!(addr3.starts_with("turkiumdev:"), "Address should have turkiumdev: prefix");
    ...
    async fn test_wallet_address_cleaning_without_prefix() {
        // Test: Wallet addresses without prefix should get turkium: prefix added
        let ctx = create_test_context().await;
        ...
        assert!(!addr_param.starts_with("turkium:"), "Test address should not have prefix initially");

        // handle_authorize will call clean_wallet which should add turkium: prefix
    ...
    fn test_share_submit_ethereumstratum_format_validation() {
        ...
        // Note: Full submission testing requires a TurkiumApi mock, so we test the format here
        ...
        let event = JsonRpcEvent::new(
            ...
            "mining.submit",
            vec![
                json!("Turkiumtest:qr8example123456789012345678901234567890123456789012345678901234567890.worker1"),
                json!("1"),
    ...
    fn test_stratum_context_summary() {
        ...
        *ctx.wallet_addr.lock() = "Turkiumtest:qr8example123456789012345678901234567890123456789012345678901234567890".to_string();
        *ctx.worker_name.lock() = "worker1".to_string();
        *ctx.remote_app.lock() = "BzMiner".to_string();
        ...
        assert_eq!(
            summary.wallet_addr, "Turkiumtest:qr8example123456789012345678901234567890123456789012345678901234567890",

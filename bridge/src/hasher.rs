use turkium_hashes::{BlockHash, HasherBase};
...
/// Turkium difficulty representation
#[derive(Debug, Clone)]
pub struct TurkiumDiff {
    pub hash_value: f64,
    pub diff_value: f64,
    pub target_value: BigUint,
}

impl Default for TurkiumDiff {
    fn default() -> Self {
...
pub fn diff_to_target_alternative(diff: f64) -> BigUint {
    ...
    // TURKPA_MAX_TARGET = 2^64 - 1
    const TURKPA_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
    let max_target_big = BigUint::from(TURKPA_MAX_TARGET);
...
pub fn stratum_difficulty_to_target_Turkium(stratum_diff: u64) -> BigUint {
    // Handle zero difficulty - return maximum target
    if stratum_diff == 0 {
        const TURKPA_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
        return BigUint::from(TURKPA_MAX_TARGET) * BigUint::from(1000u64);
    }

    // TURKPA_MAX_TARGET = 2^64 - 1
    const TURKPA_MAX_TARGET: u64 = 0xFFFFFFFFFFFFFFFF;
    let base_target = BigUint::from(TURKPA_MAX_TARGET);
...
/// Based on: https://github.com/tmrlvi/Turkium-miner/blob/bf361d02a46c580f55f46b5dfa773477634a5753/src/client/stratum.rs#L375
pub fn diff_to_target(diff: f64) -> BigUint {
    ...
    if use_stratum_alt {
        ...
        let stratum_target = stratum_difficulty_to_target_Turkium(diff_u64);
...
/// Uses turkium_hashes::BlockHash to match the working stratum implementation
/// Returns the Hash type directly (not bytes) to match working stratum code
pub fn serialize_block_header(
    block: &turkium_consensus_core::block::Block,
) -> Result<turkium_hashes::Hash, Box<dyn std::error::Error>> {
...
///
/// NOTE: Timestamp is in MILLISECONDS (as per Turkium block header format)
/// Pool uses: timestampLE.writeBigUInt64LE(timestamp) which writes u64 in LE
pub fn generate_iceriver_job_params(pre_pow_hash: &turkium_hashes::Hash, timestamp: u64) -> String {
...
/// This matches Turkium's compact difficulty format

...
use Turkium_database::{create_temp_db, load_existing_db};
use Turkium_hashes::Hash;
use Turkium_perf_monitor::{builder::Builder, counters::CountersSnapshot};
use Turkium_utils::fd_budget;
use async_channel::unbounded;
use clap::Parser;
use futures::{Future, future::try_join_all};
use itertools::Itertools;
use simulator::network::TurkiumNetworkSimulator;
use std::{collections::VecDeque, sync::Arc, time::Duration};

mod blocks_json;
pub mod simulator;

/// Turkium Network Simulator
#[derive(Parser, Debug)]
...
fn main() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "semaphore-trace")] {
            Turkium_core::log::init_logger(None, &format!("{},{}=debug", args.log_level, Turkium_utils::sync::semaphore_module_path()));
        } else {
            Turkium_core::log::init_logger(None, &args.log_level);
        }
    };
...
fn main_impl(mut args: Args) {
    ...
    let stop_perf_monitor = args.perf_metrics.then(|| {
        ...
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", Turkium_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", Turkium_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = TurkiumNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
...
fn apply_args_to_consensus_params(args: &Args, params: &mut Params) {
    ...
    if args.testnet11 {
        info!(
            "Using Turkium-testnet-11 configuration (GHOSTDAG K={}, DAA window size={}, Median time window size={})",
            params.ghostdag_k(),
...
mod tests {
    ...
    fn test_pruning_via_simpa() {
        ...
        Turkium_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        Turkium_core::panic::configure_panic();
        main_impl(args);
    }

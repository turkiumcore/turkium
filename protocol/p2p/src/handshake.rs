...
impl<'a> TurkiumdHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![TurkiumdMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![TurkiumdMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![TurkiumdMessagePayloadType::Ready]),
        }
    }

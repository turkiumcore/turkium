...
//!
//! Turkium [`Address`] implementation.
//!
//! In it's string form, the Turkium [`Address`] is represented by a `bech32`-encoded
...
/// Address prefix identifying the network type this address belongs to (such as `turk`, `turktest`, `turksim`, `turkdev`).
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[borsh(use_discriminant = true)]
pub enum Prefix {
    #[serde(rename = "turk")]
    Mainnet,
    #[serde(rename = "turktest")]
    Testnet,
    #[serde(rename = "turksim")]
    Simnet,
    #[serde(rename = "turkdev")]
    Devnet,
    #[cfg(test)]
    A,
    #[cfg(test)]
...
///
///  Turkium `Address` version (`PubKey`, `PubKey ECDSA`, `ScriptHash`)
///
/// @category Address
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[repr(u8)]
...
impl TryFrom<&str> for Address {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        match value.split_once(':') {
            Some((prefix, payload)) => {
                // Normalize prefix to lowercase for case-insensitive parsing
                let normalized_prefix = prefix.to_lowercase();
                Self::decode_payload(normalized_prefix.as_str().try_into()?, payload)
            }

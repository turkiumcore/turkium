...
final turkiumPriceProvider = Provider.autoDispose((ref) {
  return ref.watch(coingeckoTurkiumPriceProvider);
});
...
final formatedTotalFiatProvider = Provider.autoDispose((ref) {
  final balance = ref.watch(totalBalanceProvider);
  final price = ref.watch(turkiumPriceProvider);
...
final formatedTurkiumPriceProvider = Provider.autoDispose((ref) {
  final price = ref.watch(turkiumPriceProvider).price;
  final currency = ref.watch(currencyProvider);
  final symbol = ref.watch(turkiumSymbolProvider);
...
final turkiumFormatterProvider = Provider((ref) {
  final symbol = ref.watch(turkiumSymbolProvider);
  ...
  final formatter = CurrencyFormatter(
    ...
    maxDecimalDigits: TokenInfo.turkium.decimals,
    maxAmount: kMaxTurkium,
  );
...
final fiatFormatterProvider = Provider.autoDispose((ref) {
  final price = ref.watch(turkiumPriceProvider);
  final currency = ref.watch(currencyProvider);
  final maxAmount = price.price * kMaxTurkium;

    ...
    Future<void> addNode() async {
      ...
      try {
        ...
        try {
          ...
          client = TurkiumClient.url(url, isSecure: true);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = true;
        } catch (_) {
          // Fallback to insecure connection
          client = TurkiumClient.url(url, isSecure: false);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = false;
        }

        TurkiumNetwork network;
        String suffix;
        final parts = networkName.split('-');
        if (parts.length > 1) {
          network = TurkiumNetwork.tryParse(parts[1]) ?? networkForPort(port);
        ...
        final notifier = ref.read(turkiumNodeSettingsProvider.notifier);

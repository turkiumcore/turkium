...
import '../turkium/grpc/rpc.pb.dart';
...
final networkProvider = Provider((ref) {
  final config = ref.watch(turkiumNodeConfigProvider);
...
final turkiumClientProvider = Provider((ref) {
  final config = ref.watch(turkiumNodeConfigProvider);
  final inBackground = ref.watch(inBackgroundProvider);

  final client = inBackground
      ? VoidTurkiumClient()
      : TurkiumClient.url(config.url, isSecure: config.isSecure);
...
final turkiumSymbolProvider = Provider((ref) {
  final network = ref.watch(networkProvider);

  return switch (network) {
    TurkiumNetwork.mainnet => 'TUR',
    _ => 'TKAS',
  };
});

final symbolProvider = Provider.family<String, Amount>((ref, amount) {
  final kasSymbol = ref.watch(turkiumSymbolProvider);
  if (amount.tokenInfo.tokenId != TokenInfo.turkium.tokenId) {

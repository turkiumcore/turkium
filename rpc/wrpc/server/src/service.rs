...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "127.0.0.1:5202".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### TurkiumRpcHandler
///
/// [`TurkiumRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct TurkiumRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl TurkiumRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        TurkiumRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for TurkiumRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "Turkium" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<TurkiumRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(TurkiumRpcHandler::new(tasks, *encoding, core_service, options.clone()));

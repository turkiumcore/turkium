//!
//! Module implementing [`Resolver`] client for obtaining public Turkium wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public Turkium wRPC endpoints.
///
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/Turkium/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Turkium p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Turkium wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].

//! Turkium wRPC client implementation.
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("Turkium wRPC::{encoding} connecting to: {url}");
...
impl Debug for TurkiumRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TurkiumRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}
...
impl TurkiumRpcClient {
    ...
    pub fn new(
        ...
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<TurkiumRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = TurkiumRpcClient { inner };
...
#[async_trait]
impl RpcApi for TurkiumRpcClient {
    //

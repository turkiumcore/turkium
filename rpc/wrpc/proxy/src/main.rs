...
use Turkium_wrpc_server::{
    ...
    router::Router,
    server::Server,
    service::{Options, TurkiumRpcHandler},
};
use clap::Parser;
use result::Result;
use std::sync::Arc;
...
struct Args {
    ...
    /// proxy:port for gRPC server (grpc://127.0.0.1:5200)
    #[clap(name = "grpc")]
    grpc_proxy_address: Option<String>,

    // /// wRPC port
    /// interface:port for wRPC server (wrpc://127.0.0.1:5202)
    #[clap(long)]
...
async fn main() -> Result<()> {
    ...
    let rpc_handler = Arc::new(TurkiumRpcHandler::new(tasks, encoding, None, options.clone()));
    ...
    log_info!("Turkium wRPC server is listening on {}", options.listen_address);

...
struct RpcTable {
    ...
    turkiumd_request_type: Expr,
    turkiumd_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
...
impl Parse for RpcTable {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let turkiumd_request_type = iter.next().unwrap().clone();
        let turkiumd_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            turkiumd_request_type,
            turkiumd_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let turkiumd_request_type = &self.turkiumd_request_type;
        let turkiumd_response_type = &self.turkiumd_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #turkiumd_request_type, #turkiumd_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #turkiumd_request_type| {
                                Box::pin(async move {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #turkiumd_request_type, #turkiumd_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #turkiumd_request_type| {
                                Box::pin(async move {
                                    let mut response: #turkiumd_response_type = match request.payload {

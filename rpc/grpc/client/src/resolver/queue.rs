...
struct Pending {
    ...
    sender: TurkiumdResponseSender,
}

impl Pending {
    fn new(op: TurkiumdPayloadOps, request: TurkiumdRequest, sender: TurkiumdResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &TurkiumdResponse, response_op: TurkiumdPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: TurkiumdPayloadOps, request: &TurkiumdRequest) -> TurkiumdResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<TurkiumdResponse>>();
        {
    ...
    fn handle_response(&self, response: TurkiumdResponse) {
        let response_op: TurkiumdPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");

...
type TurkiumdRequestSender = async_channel::Sender<TurkiumdRequest>;
type TurkiumdRequestReceiver = async_channel::Receiver<TurkiumdRequest>;
...
struct Inner {
    ...
    // Sending to server
    request_sender: TurkiumdRequestSender,
    request_receiver: TurkiumdRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<TurkiumdResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual TurkiumdRequest to TurkiumdResponse stream
        let mut stream: Streaming<TurkiumdResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: TurkiumdPayloadOps, request: impl Into<TurkiumdRequest>) -> Result<TurkiumdResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: TurkiumdRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<TurkiumdResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = turkiumd_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = turkiumd_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }

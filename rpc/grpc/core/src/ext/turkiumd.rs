...
use crate::protowire::{
    ...
    NotifyVirtualChainChangedRequestMessage, NotifyVirtualDaaScoreChangedRequestMessage, TurkiumdRequest, TurkiumdResponse,
    turkiumd_request, turkiumd_response,
};

impl TurkiumdRequest {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        TurkiumdRequest { id: 0, payload: Some(turkiumd_request::Payload::from_notification_type(scope, command)) }
    }
...
impl turkiumd_request::Payload {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        match scope {
            Scope::BlockAdded(_) => {
                turkiumd_request::Payload::NotifyBlockAddedRequest(NotifyBlockAddedRequestMessage { command: command.into() })
            }
            Scope::NewBlockTemplate(_) => {
                turkiumd_request::Payload::NotifyNewBlockTemplateRequest(NotifyNewBlockTemplateRequestMessage {
                    command: command.into(),
                })
            }

            Scope::VirtualChainChanged(scope) => {
                turkiumd_request::Payload::NotifyVirtualChainChangedRequest(NotifyVirtualChainChangedRequestMessage {
                    command: command.into(),
                    include_accepted_transaction_ids: scope.include_accepted_transaction_ids,
                })
            }
            Scope::FinalityConflict(_) => {
                turkiumd_request::Payload::NotifyFinalityConflictRequest(NotifyFinalityConflictRequestMessage {
                    command: command.into(),
                })
            }
            ...
            Scope::UtxosChanged(scope) => turkiumd_request::Payload::NotifyUtxosChangedRequest(NotifyUtxosChangedRequestMessage {
                addresses: scope.addresses.iter().map(|x| x.into()).collect::<Vec<String>>(),
                command: command.into(),
            }),
            Scope::SinkBlueScoreChanged(_) => {
                turkiumd_request::Payload::NotifySinkBlueScoreChangedRequest(NotifySinkBlueScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::VirtualDaaScoreChanged(_) => {
                turkiumd_request::Payload::NotifyVirtualDaaScoreChangedRequest(NotifyVirtualDaaScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::PruningPointUtxoSetOverride(_) => {
                turkiumd_request::Payload::NotifyPruningPointUtxoSetOverrideRequest(NotifyPruningPointUtxoSetOverrideRequestMessage {
                    command: command.into(),
                })
            }
        }
    ...
    pub fn is_subscription(&self) -> bool {
        use crate::protowire::turkiumd_request::Payload;
        matches!(
            self,
...
impl turkiumd_response::Payload {
    pub fn is_notification(&self) -> bool {
        use crate::protowire::turkiumd_response::Payload;
        match self {

...
impl NetworkType {
    pub fn default_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 5200,
            NetworkType::Testnet => 5201,
            NetworkType::Simnet => 5202,
            NetworkType::Devnet => 5203,
        }
    }

    pub fn default_borsh_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 5202,
            NetworkType::Testnet => 5203,
            NetworkType::Simnet => 5204,
            NetworkType::Devnet => 5205,
        }
    }

    pub fn default_json_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 5204,
            NetworkType::Testnet => 5205,
            NetworkType::Simnet => 5206,
            NetworkType::Devnet => 5207,
        }
    }
...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'Turkium'.")]
...
///
/// NetworkId is a unique identifier for a Turkium network instance.
...
impl NetworkId {
    ...
    pub fn default_p2p_port(&self) -> u16 {
        ...
        match self.network_type {
            NetworkType::Mainnet => 5206,
            NetworkType::Testnet => match self.suffix {
                Some(10) => 5207,
                Some(11) => 5208,
                None | Some(_) => 5209,
            },
            NetworkType::Simnet => 5210,
            NetworkType::Devnet => 5211,
        }
    }
    ...
    /// Returns a textual description of the network prefixed with `Turkium-`
    pub fn to_prefixed(&self) -> String {
        format!("Turkium-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("Turkium-") {

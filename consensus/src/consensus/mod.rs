...
impl Consensus {
    pub fn new(
        ...
        // Only process genesis if it hasn't been processed yet (check if genesis header exists in store)
        if config.process_genesis {
            let genesis_hash = params.genesis.hash;
            let genesis_exists = storage.headers_store.has(genesis_hash).unwrap_or(false);

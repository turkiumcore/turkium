...
impl CoinbaseManager {
    pub fn new(
        ...
        // In a 10 BPS network, the induced increase in total rewards is 51 TURK (see tests::calc_high_bps_total_rewards_delta())
...
mod tests {
    ...
    fn calc_high_bps_total_rewards_delta() {
        ...
        println!("Total rewards: {} sompi => {} TURK", total_rewards, total_rewards / SOMPI_PER_TURKIUM);
        println!("Total high bps rewards: {} sompi => {} TURK", total_high_bps_rewards, total_high_bps_rewards / SOMPI_PER_TURKIUM);
        println!("Delta: {} sompi => {} TURK", delta, delta / SOMPI_PER_TURKIUM as i64);
    }
    ...
    /// `cargo test --release --package Turkium-consensus --lib -- processes::coinbase::tests::verify_crescendo_emission_schedule --exact --nocapture --ignored`
    #[test]
    #[ignore = "long"]
    fn verify_crescendo_emission_schedule() {
        ...
        for network_id in [NetworkId::new(NetworkType::Mainnet)] {
            ...
            for activation in activations {
                ...
                let diff = (new_total as i64 - baseline_total as i64) / SOMPI_PER_TURKIUM as i64;
                assert!(diff.abs() <= 51, "activation: {}", activation);
                println!("DIFF (TURK): {}", diff);
            }
        }
    }

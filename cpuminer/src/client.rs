use crate::{
    miner::MinerManager,
    proto::{
        rpc_client::RpcClient, turkiumd_message::Payload, GetBlockTemplateRequestMessage, GetInfoRequestMessage,
        TurkiumdMessage,
    },
...
#[allow(dead_code)]
pub struct TurkiumdHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<TurkiumdMessage>,
    stream: Streaming<TurkiumdMessage>,
...
impl TurkiumdHandler {
    ...
    pub async fn client_send(&self, msg: impl Into<TurkiumdMessage>) -> Result<(), SendError<TurkiumdMessage>> {
        self.send_channel.send(msg.into()).await
    }

    pub async fn client_get_block_template(&mut self) -> Result<(), SendError<TurkiumdMessage>> {
    ...
    pub async fn listen(&mut self, miner: &mut MinerManager, shutdown: ShutdownHandler) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            ...
            match msg.payload {
                ...
                None => warn!("turkiumd message payload is empty"),
            }
        }
        Ok(())
    }

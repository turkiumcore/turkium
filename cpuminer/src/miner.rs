use crate::{
    pow,
    proto::{RpcBlock, TurkiumdMessage},
    swap_rust::WatchSwap,
...
pub struct MinerManager {
    ...
    send_channel: Sender<TurkiumdMessage>,
...
impl MinerManager {
    ...
    pub fn launch_cpu_miner(
        ...
        #[cold]
        fn found_block(send_channel: &Sender<TurkiumdMessage>, block: RpcBlock) -> Result<(), Error> {
            let block_hash = block.block_hash().expect("We just got it from the state, we should be able to hash it");
            send_channel.blocking_send(TurkiumdMessage::submit_block(block))?;

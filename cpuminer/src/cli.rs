...
pub struct Opt {
    ...
    #[clap(short = 's', long = "pool-address", display_order = 1)]
    /// The IP/hostname of the Stratum pool [default: auto-detect from mining address]
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        // Auto-detect pool address from mining address if not provided
        let address = if let Some(addr) = &self.pool_address {
            addr.clone()
        } else {
            // Auto-detect server based on mining address prefix
            if self.mining_address.starts_with("turk:") {
                // Mainnet - use main pool server
                "188.132.197.20".to_string()
            } else if self.mining_address.starts_with("turktest:") {
                // Testnet - use main pool server
                "188.132.197.20".to_string()
            } else if self.mining_address.starts_with("turkdev:") {
                // Devnet - use main pool server
                "188.132.197.20".to_string()
            } else {
        ...
        // Pool mode - just IP:PORT format
        let port = self.port();
        let final_address = format!("{}:{}", address, port);

        self.pool_address = Some(final_address.clone());
        
        log::info!("Connecting to Stratum pool at {}", final_address);
    ...
    fn port(&mut self) -> u16 {
        *self.port.get_or_insert(3333)  // Pool default port
    }

//!
//! API trait for interfacing with the Turkium wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `Turkium`.
    ///
    ...
    /// applications such as KDX and Turkiumnet.io web wallet using a deprecated derivation path

 ...
 * Wallet API for interfacing with Rusty Turkium Wallet implementation.
 */
"#;
...
declare! {
     ...
     * with the default filename `Turkium`.
     * 
     * @category Wallet API
     */
...
try_from! (args: IAccountsCreateRequest, AccountsCreateRequest, {
    ...
    let account_create_args = match kind.as_str() {
        ...
        _ => {
            return Err(Error::custom("only BIP32/Turkium-keypair-standard accounts are currently supported"));
        }
    };

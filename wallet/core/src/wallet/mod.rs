...
impl Wallet {
    ...
    pub fn try_with_wrpc(store: Arc<dyn Interface>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Wallet> {
        let rpc_client =
            Arc::new(TurkiumRpcClient::new_with_args(WrpcEncoding::Borsh, Some("wrpc://127.0.0.1:5202"), resolver, network_id, None)?);
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<TurkiumRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<TurkiumRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<TurkiumRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<TurkiumRpcClient>().ok()).unwrap()
    }
    ...
    pub async fn import_Turkiumwallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
    ...
    pub async fn import_Turkiumwallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_Turkiumwallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_Turkiumwallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
...
mod test {
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let address = Address::try_from("Turkiumtest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("Turkiumtest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;

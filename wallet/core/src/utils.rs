//!
//! Turkium value formatting and parsing utilities.
//!
...
pub fn try_Turkium_str_to_sompi_i64<S: Into<String>>(s: S) -> Result<Option<i64>> {
    ...
    let amount = amount.parse::<f64>()? * SOMPI_PER_TURKIUM as f64;
...
fn str_to_sompi(amount: &str) -> Result<u64> {
    let Some(dot_idx) = amount.find('.') else {
        return Ok(amount.parse::<u64>()? * SOMPI_PER_TURKIUM);
    };
    let integer = amount[..dot_idx].parse::<u64>()? * SOMPI_PER_TURKIUM;

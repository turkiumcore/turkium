...
use turkium_wrpc_client::TurkiumRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<TurkiumRpcClient>> {
        self.rpc_api().clone().downcast_arc::<TurkiumRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to turkiumd: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");

...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], None, Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Turkium(20.0),
            output_count: 1,
...
#[test]
fn test_generator_compound_200k_10turk_transactions() -> Result<()> {
    generator(
        test_network_id(),
        &[10.0; 200_000],
        &[],
        None,
        Fees::sender(Turkium(5.0)),
        [(output_address, Turkium(190_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .validate()
...
#[test]
fn test_generator_fee_rate_compound_200k_10turk_transactions() -> Result<()> {
    generator(
        test_network_id(),
        &[10.0; 200_000],
        &[],
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Turkium(total - 10.0))];
    generator(test_network_id(), &inputs, &[], None, Fees::sender(Turkium(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Turkium(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], None, Fees::sender(Turkium(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Turkium(5.0)),
        [(output_address, Turkium(1.0)), (output_address, Turkium(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Turkium(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Turkium(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Turkium(5.0)),
        [(output_address, Turkium(10.0)), (output_address, Turkium(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Turkium(5.0)), [(output_address, Turkium(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Turkium(0.0)), [(output_address, Turkium(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Turkium(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Turkium(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Turkium(5.0)),
            priority_fees: FeesExpected::sender(Turkium(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[],
        None,
        Fees::receiver(Turkium(5.0)),
        // [(output_address, Turkium(100.0))].as_slice(),
        [(output_address, Turkium(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Turkium(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Turkium(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Turkium(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], None, Fees::sender(Turkium(5.0)), [(output_address, Turkium(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_1k_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        None,
        Fees::sender(Turkium(5.0)),
        [(output_address, Turkium(9_000.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .drain(
    ...
    .fetch(&Expected {
        is_final: false,
        input_count: 21,
        aggregate_input_value: Turkium(210.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
...
fn test_generator_inputs_32k_outputs_2_fees_exclude() -> Result<()> {
    ...
    generator(
        ...
        &[],
        None,
        Fees::sender(Turkium(10_000.0)),
        [(output_address, Turkium(f * 32_747.0 - 10_001.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .accumulate(379)
...
fn test_generator_fan_out_1() -> Result<()> {
    ...
    //     1.00000000,
    //     87.23579967,
    // ], &[], None, Fees::sender(Turkium(1.0)), [
    //     (output_address, Turkium(0.20000000)),
    //     (output_address, Turkium(0.25000000)),
    //     (output_address, Turkium(0.21000000)),
    // ].as_slice())
    //     .unwrap()
    //     .harness()
    //     // .accumulate(1)
    ...
    //         is_final: true,
    //         input_count: 2,
    //         aggregate_input_value: Turkium(1.00000000 + 87.23579967),
    //         output_count: 4,
    //         priority_fees: FeesExpected::receiver(Turkium(1.0)),
    //         // priority_fees: FeesExpected::None,
    //     })
    //     .finalize();

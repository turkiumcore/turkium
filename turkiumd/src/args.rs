...
pub fn cli() -> Command {
    ...
    let cmd = Command::new("turkiumd")
        ...
        .arg(arg!(-b --appdir <DATA_DIR> "Directory to store data.").env("TURKPAD_APPDIR"))
        .arg(arg!(--logdir <LOG_DIR> "Directory to log output.").env("TURKPAD_LOGDIR"))
        .arg(arg!(--nologfiles "Disable logging to files.").env("TURKPAD_NOLOGFILES"))
        .arg(
            Arg::new("async_threads")
                .short('t')
                .long("async-threads")
                .env("TURKPAD_ASYNC_THREADS")
        ...
        .arg(
            Arg::new("rpcmaxclients")
                .long("rpcmaxclients")
                .env("TURKPAD_RPCMAXCLIENTS")
                .value_name("rpcmaxclients")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
                .help("Max number of RPC clients for standard connections (default: 128)."),
        )
        .arg(arg!(--"reset-db" "Reset database before starting node. It's needed when switching between subnetworks.").env("TURKPAD_RESET_DB"))
        .arg(arg!(--"enable-unsynced-mining" "Allow the node to accept blocks from RPC while not synced (this flag is mainly used for testing)").env("TURKPAD_ENABLE_UNSYNCED_MINING"))
        .arg(
            Arg::new("enable-mainnet-mining")
                .long("enable-mainnet-mining")
                .env("TURKPAD_ENABLE_MAINNET_MINING")
                .action(ArgAction::SetTrue)
                .hide(true)
                .help("Allow mainnet mining (currently enabled by default while the flag is kept for backwards compatibility)"),
        )
        .arg(arg!(--utxoindex "Enable the UTXO index").env("TURKPAD_UTXOINDEX"))
        .arg(
            Arg::new("max-tracked-addresses")
                .long("max-tracked-addresses")
                .env("TURKPAD_MAX_TRACKED_ADDRESSES")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(arg!(--testnet "Use the test network").env("TURKPAD_TESTNET"))
        .arg(
            Arg::new("netsuffix")
                .long("netsuffix")
                .env("TURKPAD_NETSUFFIX")
                .value_name("netsuffix")
                .require_equals(true)
                .value_parser(clap::value_parser!(u32))
                .help("Testnet network suffix number"),
        )
        .arg(arg!(--devnet "Use the development test network").env("TURKPAD_DEVNET"))
        .arg(arg!(--simnet "Use the simulation test network").env("TURKPAD_SIMNET"))
        .arg(arg!(--archival "Run as an archival node: avoids deleting old block data when moving the pruning point (Warning: heavy disk usage)").env("TURKPAD_ARCHIVAL"))
        .arg(arg!(--sanity "Enable various sanity checks which might be compute-intensive (mostly performed during pruning)").env("TURKPAD_SANITY"))
        .arg(arg!(--yes "Answer yes to all interactive console questions").env("TURKPAD_NONINTERACTIVE"))
        .arg(
            Arg::new("user_agent_comments")
                .long("uacomment")
                .env("TURKPAD_USER_AGENT_COMMENTS")
                .action(ArgAction::Append)
                .require_equals(true)
        ...
        .arg(arg!(--"perf-metrics" "Enable performance metrics: cpu, memory, disk io usage").env("TURKPAD_PERF_METRICS"))
        .arg(
            Arg::new("perf-metrics-interval-sec")
                .long("perf-metrics-interval-sec")
                .env("TURKPAD_PERF_METRICS_INTERVAL_SEC")
                .require_equals(true)
                .value_parser(clap::value_parser!(u64))
                .help("Interval in seconds for performance metrics collection."),
        )
        .arg(arg!(--"disable-upnp" "Disable upnp").env("TURKPAD_DISABLE_UPNP"))
        .arg(arg!(--"nodnsseed" "Disable DNS seeding for peers").env("TURKPAD_NODNSSEED"))
        .arg(arg!(--"nogrpc" "Disable gRPC server").env("TURKPAD_NOGRPC"))
        .arg(
            Arg::new("ram-scale")
                .long("ram-scale")
                .env("TURKPAD_RAM_SCALE")
                .require_equals(true)
...
a large RAM (~64GB) can set this value to ~3.0-4.0 and gain superior performance especially for syncing peers faster"),
        ...
        .arg(
            Arg::new("rocksdb-cache-size")
                ...
                .env("TURKPAD_ROCKSDB_CACHE_SIZE")
                .require_equals(true)
                .value_parser(clap::value_parser!(usize))
...
/*
  ...
  -d, --loglevel=                           Logging level for all subsystems {trace, debug, info, warn, error,
      ...
      --minrelaytxfee=                      The minimum transaction fee in TURK/kB to be considered a non-zero fee.
                                            (default: 1e-05)

...
impl From<&TurkiumCli> for Arc<Terminal> {
    fn from(ctx: &TurkiumCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<TurkiumCli> for TurkiumCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for TurkiumCli {
...
impl TurkiumCli {
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<TurkiumRpcClient>> {
...
impl cli::Context for TurkiumCli {
...
pub async fn turkium_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    ...
    let cli = TurkiumCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Turkium Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));

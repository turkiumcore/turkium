...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<TurkiumCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<TurkiumCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<TurkiumCli>) -> Result<()> {
        ...
        if binaries.is_empty() {
            ...
            if let Some(selection) = ctx.term().select("Please select Turkium-cpu-miner binary", &binaries).await? {

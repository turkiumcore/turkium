...
#[async_trait]
impl DefaultSettings for TurkiumdSettings {
...
pub struct Node {
    settings: SettingsStore<TurkiumdSettings>,
    mute: Arc<AtomicBool>,
...
impl Handler for Node {
    ...
    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        ...
        if let Some(mute) = self.settings.get(TurkiumdSettings::Mute) {
    ...
    async fn handle(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, cmd: &str) -> cli::Result<()> {
        let ctx = ctx.clone().downcast_arc::<TurkiumCli>()?;
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<TurkiumCli>) -> Result<TurkiumdConfig> {
        let location: String = self
            .settings
            .get(TurkiumdSettings::Location)
        ...
        let mute = false;
        let config = TurkiumdConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<TurkiumCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<TurkiumCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();
    ...
    pub async fn handle_event(&self, ctx: &Arc<TurkiumCli>, event: Event) -> Result<()> {
        let term = ctx.term();

...
const DEFAULT_SEND_AMOUNT: u64 = 10 * SOMPI_PER_TURKIUM;
...
pub fn cli() -> Command {
    Command::new("turk-txgen")
        .about(format!("{} (turk-txgen) v{}", env!("CARGO_PKG_DESCRIPTION"), version()))
        ...
        .arg(
            Arg::new("rpcserver")
                ...
                .help("RPC server (defaults: testnet=5201, devnet=5201"),
        )
        .arg(
            Arg::new("threads")
                .long("threads")
...
async fn main() {
    ...
    let mut log_message = format!(
        "Using Turkium Transaction Generator with:\n\
        \tnetwork: {}\n\
        \tprivate key: {}\n\
        \tfrom address: {}",
        args.network,
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(turkium_addr);
    let inputs = utxos
        .iter()
